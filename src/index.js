...
<section class="mast">
<div class="eyebrow">Ocean Liner Curator · Site Maintenance</div>
...
<input id="search" placeholder="Search URL, page, context…"><select id="severity"><option value="">All severities</option><option value="broken">Broken</option><option value="warning">Warning</option><option value="good">Good</option></select><select id="location"><option value="">All locations</option><option value="source">Sources only</option><option value="body">Body links only</option></select><button class="secondary" id="exportBtn" disabled>Export CSV</button>
</section>
...
async function run(){if(state.running)return;state.running=true;state.queue=[];state.seen=new Set();state.pages=[];state.rows=[];$('exportBtn').disabled=true;render();const start=$('startUrl').value.trim();state.queue.push(start);try{while(state.queue.length){const pageUrl=state.queue.shift();if(state.seen.has(pageUrl))continue;state.seen.add(pageUrl);setStatus('Inspecting '+pageUrl);const page=await api('/api/page',{url:pageUrl});state.pages.push(page);if($('scope').value==='site')for(const link of page.internalLinks||[])if(!state.seen.has(link))state.queue.push(link);for(const link of page.auditLinks||[]){setStatus('Checking '+link.url);const check=await api('/api/check',{url:link.url});let replacement='';if(check.severity==='broken'){const suggestion=await api('/api/suggest',{url:link.url});replacement=suggestion.suggestions?.[0]?.url||'';}state.rows.push({page_url:page.finalUrl,page_title:page.title,checked_url:link.url,anchor_text:link.label,context:link.context,in_sources:link.inSources,status:check.status??'',category:check.category,severity:check.severity,final_url:check.finalUrl||'',replacement_url:replacement});render();}updateStats();}setStatus('Audit complete.');$('exportBtn').disabled=!state.rows.length;}catch(e){setStatus(e.message)}finally{state.running=false;}}
function render(){const q=$('search').value.toLowerCase(),sev=$('severity').value,loc=$('location').value;const rows=state.rows.filter(r=>(!q||[r.page_url,r.checked_url,r.context,r.page_title].join(' ').toLowerCase().includes(q))&&(!sev||r.severity===sev)&&(!loc||(loc==='source'?r.in_sources:!r.in_sources)));$('rows').innerHTML=rows.map(r=>'<tr><td><a target="_blank" href="'+esc(r.page_url)+'">'+esc(r.page_title||r.page_url)+'</a></td><td><a target="_blank" href="'+esc(r.checked_url)+'">'+esc(r.anchor_text||r.checked_url)+'</a></td><td>'+(r.in_sources?'Sources':'Body')+'</td><td><span class="pill '+esc(r.severity)+'">'+esc(r.category)+'</span></td><td>'+esc(r.status)+'</td><td>'+esc(r.final_url)+'</td><td class="small">'+esc(r.context)+'</td><td class="replacement">'+(r.replacement_url?'<a target="_blank" href="'+esc(r.replacement_url)+'">'+esc(r.replacement_url)+'</a>':'')+'</td></tr>').join('');updateStats();}
function updateStats(){$('sPages').textContent=state.pages.length;$('sLinks').textContent=state.rows.length;$('sGood').textContent=state.rows.filter(r=>r.severity==='good').length;$('sWarning').textContent=state.rows.filter(r=>r.severity==='warning').length;$('sBroken').textContent=state.rows.filter(r=>r.severity==='broken').length;$('sSources').textContent=state.rows.filter(r=>r.in_sources).length;$('progress').style.width=(state.running?Math.min(96,Math.max(3,state.seen.size/(state.seen.size+state.queue.length+1)*100)):state.rows.length?'100%':'0%')}
function setStatus(t){$('status').textContent=t;updateStats()}
